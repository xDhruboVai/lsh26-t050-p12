    ...
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-[22px] font-bold tracking-tight">
          Rest of {monthLabelShort(f.month)}
        </h1>
      ...
      <Card className="card-lead">
        ...
        <div className="mt-4 border-t border-rule pt-3">
          <div className="flex justify-between py-1 text-[13.5px]">
            ...
            <span className="num">
              {fmt(f.spentThisMonthPaisa, { paisa: false })}
            </span>
          </div>
          <div className="flex justify-between py-1 text-[13.5px]">
          ...
          <div className="flex justify-between py-1 text-[13.5px]">
            ...
            <span className="num">
              {fmt(f.recurringDuePaisa, { paisa: false })}
            </span>
          </div>
          <div className="mt-1 flex justify-between border-t border-rule pt-2 text-[14px] font-semibold">
            ...
            <span className="num">
              {fmt(f.projectedMonthTotalPaisa, { paisa: false })}
            </span>
          </div>
        </div>

        <p className="mt-3 text-[11.5px] leading-relaxed text-ink3">
          Method: a category and shop that appeared in{" "}
          {monthLabel(ledger.months.last)} but not yet this month is treated as
          a bill still due, at last month&rsquo;s amount. Everything else runs
      ...
      {/* Insights */}
      <Card>
        <SectionTitle hint={`${f.insights.length} from your numbers`}>
          What stands out
        </SectionTitle>
      ...
      <Card>
        ...
        <p className="mb-2 text-[11.5px] leading-relaxed text-ink3">
          Auto-recurring marks the same shop in both months when the totals are
          within 10%.
        </p>
        {f.recurring.length === 0 ? (
          <p className="text-[14px] text-ink2">
            Nothing repeated from {monthLabel(ledger.months.last)} yet, so the
            whole month is treated as variable spending.
          </p>
        ) : (
          <ul>
              ...
              >
                ...
                <div className="flex flex-none items-center gap-2">
                  <span className="num text-[13.5px]">
                    {fmt(r.expectedPaisa, { paisa: false })}
                  </span>
                  <Chip tone={r.seenThisMonth ? "good" : "warn"}>
                    {r.autoRecurring
                      ? "auto recurring"
                      : r.seenThisMonth
                        ? "paid"
