...
const cases = (
  JSON.parse(readFileSync("data/cases.json", "utf8")) as {
...
async function main() {
  const users =
    (await sql`SELECT id FROM users WHERE email_lower = 'demo@ledger.app'`) as any[];
  ...
  const page = async (p: string) =>
    (await fetch(BASE + p, { headers: { cookie } })).text();
  ...
  ok(
    ...
    Boolean(ex.confidence && "amount" in ex.confidence),
  );
  ok(
    "reads the grand total, not the subtotal",
    ex.amount_bdt === "3475.00",
    String(ex.amount_bdt),
  );
  ok(
    "reads a day/month date correctly",
    ex.date === "2026-09-14",
    String(ex.date),
  );
  ...
  ok(
    ...
    /Spent this month/i.test(dash) && /Salary/i.test(dash),
  );
  ...
  ok(
    "partial month is labelled, not extrapolated",
    /still running/i.test(dash),
  );
  ...
  ok(
    "dates derive from the forecast",
    /not from target divided by contribution/i.test(pk),
  );
  ok("DPS rule stated on screen", /How DPS is calculated/i.test(pk));
  ok("a new pocket can be created", /Create a savings pocket/i.test(pk));
  ok(
    "category what-if control is present",
    /What if you cut one category/i.test(pk),
  );

  const profile = await page("/profile");
  ok("private case JSON can be pasted", /Paste case JSON/i.test(profile));
  ok("public case picker is present", /Load case/i.test(profile));
  ...
  ok(
    "an unreadable amount comes back null",
    bl.amount_bdt === null,
    String(bl.amount_bdt),
  );
  ok(
    "and is scored zero",
    bl.confidence?.amount === 0,
    JSON.stringify(bl.confidence),
  );
  ok(
    "the receipt above was NOT blanked",
    ex.amount_bdt === "3475.00",
    String(ex.amount_bdt),
  );
  ...
  let minInsights = 99;
  let allInsightsNameCategory = true;
  let autoRecurring = 0;
  for (const raw of cases) {
    ...
    allInsightsNameCategory &&= f.insights.every(
      (i) =>
        i.category !== undefined && CATEGORIES.some((c) => i.text.includes(c)),
    );
    autoRecurring += f.recurring.filter((r) => r.autoRecurring).length;
  }
  ok(
    "at least 3 insights on every case",
    minInsights >= 3,
    `min ${minInsights}`,
  );
  ok(
    "all 25 cases give different insights",
    sets.size === cases.length,
    `${sets.size}/${cases.length}`,
  );
  ok("every insight names a category", allInsightsNameCategory);
  ok(
    "similar two-month expenses are auto-marked recurring",
    autoRecurring > 0,
    `${autoRecurring} marks`,
  );
  ...
  for (const raw of cases) {
    ...
    for (const p of r.plans) {
      ...
      const naiveMonths = Math.ceil(
        p.targetPaisa / Math.max(1, p.monthlyContribPaisa),
      );
      if (p.monthsToComplete !== naiveMonths) derived++;
      else naive++;
    }
  }
  ok(
    "completion differs from target/contribution somewhere",
    derived > 0,
    `${derived} forecast-driven, ${naive} coincidental`,
  );
  ...
  for (const raw of cases) {
    ...
    if (
      f.spentThisMonthPaisa + f.restOfMonthPaisa !==
      f.projectedMonthTotalPaisa
    )
      reconciles = false;
    if (s.totalPaisa !== f.spentThisMonthPaisa) reconciles = false;
    if (!Number.isInteger(f.projectedLeftPaisa)) reconciles = false;
    if (f.variableSpentPaisa !== f.spentThisMonthPaisa) reconciles = false;
  }
